...
def manifest():
    resp = send_file(
        os.path.join(BASE_PATH, "manifest.json"), mimetype="application/manifest+json"
    )
...
def service_worker():
    resp = send_file(
        os.path.join(BASE_PATH, "service-worker.js"), mimetype="application/javascript"
    )
    resp.headers["Cache-Control"] = "no-cache"
...
def _save_and_load(file):
    ...
    merged_available = list(_builtin_available) + [
        m for m in zip_available if m not in _builtin_models
    ]
...
def get_current_presentation():
    if current_presentation["file"] and os.path.exists(current_presentation["file"]):
        return send_file(
            current_presentation["file"],
            as_attachment=True,
            download_name="presentation.zip",
        )
...
def serve_zip_asset(filepath):
    ...
    if filepath in _temp_assets:
        ...
        resp.headers["Access-Control-Allow-Origin"] = "*"
        return resp
    if not current_presentation.get("file") or not os.path.exists(
        current_presentation["file"]
    ):
        return jsonify({"error": "No presentation loaded"}), 404
    try:
...
def create_survey():
    ...
    url = (
        f"/mcq/{survey_id}"
        if is_mcq
        else (f"/wordcloud/{survey_id}" if is_wordcloud else f"/survey/{survey_id}")
    )
    return jsonify(
        {
            "survey_id": survey_id,
...
def analyze_survey(survey_id):
    ...
    try:
        ...
        call_kwargs = (
            {"api_key": api_key}
            if api_key and "api_key" in inspect.signature(model_func).parameters
            else {}
        )
        summaries = model_func(
            [r["text"] for r in responses], num_summaries, **call_kwargs
        )
        if not isinstance(summaries, list) or len(summaries) != num_summaries:
            return jsonify(
                {"error": f"Model must return a list of {num_summaries} summaries"}
            ), 500
        for i, item in enumerate(summaries):
            if (
                ...
                return jsonify(
                    {"error": f"Summary {i} must be a (str, int) tuple"}
                ), 500
        return jsonify(
            {
                "summaries": [
                    {"summary": s[0], "num_respondents": s[1]} for s in summaries
                ],
                "model": model_name,
...
def start_survey_server():
    ...
    if survey_server_process.poll() is not None:
        ...
        return jsonify(
            {"success": False, "error": f"Server failed to start: {stderr}"}
        ), 500
    return jsonify(
        {"success": True, "url": "http://localhost:5001", "message": "Started"}
    )
...
def handle_widget_state(data):
    ...
    emit(
        "widget_state",
        {"widgetId": widget_id, "state": state},
        broadcast=True,
        include_self=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--http",
        action="store_true",
        help="Force plain HTTP (disables camera widget on LAN)",
    )
